<?php
    ...
    $countEndp = "http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/api/vehicle/countVehiclesByType.php?type=2";
    $numberOfRecords = file_get_contents($countEndp);

    $totalNoPages = ceil($numberOfRecords / $recordsPerPage);

    //Get this page's results
    $endp = "http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/api/vehicle/getAllVehiclesByType.php?type=2&page=$pageNo";
    $jsonResult = file_get_contents($endp);
    $convertibles = json_decode($jsonResult, true);
...
<html lang="en">
    ...
    <body>
        ...
        <div class="container">
            <h2 class='d-flex align-items-center mb-3'><i class='material-icons text-info mr-2'>all</i>Convertibles</h2>
            <h6>Browse all the Convertibles we currently have for sale</h6>
            <div class="row row-cols-1 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 g-4">
                <?php
                    foreach($convertibles as $car) {
                        $vehicleID = $car['ID'];
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    <li class="<?php if($pageNo==1){echo "page-item disabled";}else{echo "page-item";}?>">
                        <a class="page-link" href="http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/vehicle/allconvertibles.php?page=1" tabindex="-1" aria-disabled="true">First</a>
                    </li>
                    <li class="<?php if($pageNo<=1){echo "page-item disabled";}else{echo "page-item";}?>">
                        <a class="page-link" href="<?php if($pageNo<=1){echo "#";}else{echo "http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/vehicle/allconvertibles.php?page=$prevPage";}?>">Previous</a>
                    </li>
                    <li class="<?php if($pageNo>=$totalNoPages){echo "page-item disabled";}else{echo "page-item";}?>">
                        <a class="page-link" href="<?php if($pageNo>=$totalNoPages){echo "#";}else{echo "http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/vehicle/allconvertibles.php?page=$nextPage";}?>">Next</a>
                    </li>
                    <li class="<?php if($pageNo==$totalNoPages){echo "page-item disabled";}else{echo "page-item";}?>">
                        <a class="page-link" href="<?php echo "http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/vehicle/allconvertibles.php?page=$totalNoPages";?>">Last</a>
                    </li>
                </ul>
            </nav>
