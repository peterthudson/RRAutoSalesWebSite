<?php
    //Gather information from the form and store each item in variables

    session_start();

    $manu = $_POST["manufacturer"];
    $condition = $_POST["condition"];
    $year = $_POST["year"];
    $vehType = $_POST["vehType"];
    $driveType = $_POST["driveType"];
    $colour = $_POST["colour"];
    $region = $_POST["region"];
    $cylinders = $_POST["cylinders"];
    $fuelType = $_POST["fuelType"];
    $title = $_POST["title"];
    $transmission = $_POST["transmission"];
    $lat = $_POST["lat"];
    $long = $_POST["long"];
    $odometer = $_POST["odometer"];
    $vin = $_POST["vin"];
    $price = $_POST["price"];
    $model = $_POST["model"];
    $currentuser = $_SESSION["user_id"];

    //Construct the endpoint
    $url = 'http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/api/vehicle/newVehicle.php';
    
    //Construct the data to send
    $postdata = http_build_query(

        array(
            'manufacturer' => $manu,
            'condition' => $condition,
            'vehType' => $vehType,
            'year' => $year,
            'driveType' => $driveType,
            'colour' => $colour,
            'region' => $region,
            'cylinders' => $cylinders,
            'fuelType' => $fuelType,
            'title' => $title,
            'transmission' => $transmission,
            'lat' => $lat,
            'long' => $long,
            'odometer' => $odometer,
            'vin' => $vin,
            'price' => $price,
            'model' => $model,
            'user' => $currentuser,
        )
    );

    $options = array(
        'http' => array(
            'method' => 'POST',
            'header' =>  'Content-Type: application/x-www-form-urlencoded',
            'content' => $postdata
        )
    );
        
    $context  = stream_context_create($options);
    $result = file_get_contents($url, false, $context);

?>


<!doctype html>

<html lang="en">

    <head>

        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <!-- CSS -->
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BmbxuPwQa2lc/FVzBcNJ7UAyJxM6wuqIj61tLrc4wSX0szH/Ev+nYRRuWlolflfl" crossorigin="anonymous">
        <link href="/rrautosales/css/mystyles.css" rel="stylesheet">

        <!-- favicon -->
        <link rel="icon" href="/rrautosales/img/favicon.png" type="image/png" sizes="16x16">

        <!-- Icons from ionicons.com -->
        <script type="module" src="https://unpkg.com/ionicons@5.4.0/dist/ionicons/ionicons.esm.js"></script>
        <script nomodule="" src="https://unpkg.com/ionicons@5.4.0/dist/ionicons/ionicons.js"></script>

        <title>Processing</title>
    </head>

    <body>
        
        <!-- Add the Navbar to the top of the page -->
        <?php
            $headerpath = $_SERVER['DOCUMENT_ROOT'] . "/rrautosales/templates/navbar.php";
            include($headerpath);
        ?>

        <div class="container-fluid" id=mainbodycontent>
            <div class="container">
                <div class="row">
                    <div class="col-sm-9 col-md-7 col-lg-5 mx-auto">
                        <div class="card card-signin my-5 text-center">
                            <div class="card-body text-alight-center justify-content-center">
                                <?php 
                                    if($result == 1) {
                                        echo "<h5 class='card-title'>New Vehicle Listing Successful</h5>";
                                        echo "<img src='http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/img/tick.PNG'>";
                                        echo "<div class='card-text text-center' style='margin-top:10px'><p>Your $model is now listed for sale</p></div>";
                                        echo "<a href='http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/user/mylistedvehicles.php?page=1' class='btn btn-primary rounded-pill' tabindex='-1' role='button' aria-disabled='true'>My Listings</a>";
                                    } else {
                                        echo "<h5 class='card-title'>New Vehicle Listing Error</h5>";
                                        echo "<img src='http://phudson03.lampt.eeecs.qub.ac.uk/rrautosales/img/error.PNG'>";
                                        echo "<div class='card-text' style='margin-top:10px'>An error has occurred, please check the information entered in the form and try again
                                            <p>$result</p>
                                            <p>$region</p></div>";
                                    }
                                    
                                ?>                     
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Add the Footer to the bottom of the page -->
        <?php
            $footerpath = $_SERVER['DOCUMENT_ROOT'] . "/rrautosales/templates/footer.php";
            include($footerpath);
        ?>

        <!-- Javascript -->
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/js/bootstrap.bundle.min.js" integrity="sha384-b5kHyXgcpbZJO/tY9Ul7kGkf1S0CWuKcCD38l8YkeH8z8QjE0GmW1gYU5S9FOnJ0" crossorigin="anonymous"></script>

    </body>
</html>
